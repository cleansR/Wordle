...
[
    ...
    'nails','naked','named','names','nasal','nasty','naval','needs','nerve','never','newer','newly','nexus','nicer','niche','night','ninja','ninth','noble','nodes','noise','noisy','norms','north','notch','noted','notes','novel','nurse','nylon',
    
    'oasis','occur','ocean','offer','often','older','olive','omega','onion','onset','opens','opera','opted','optic','orbit','order','organ','other','ought','ounce','outer','owned','owner','oxide',

    'packs','pages','pains','paint','pairs','panel','panic','pants','paper','parks','parts','party','pasta','paste','patch','paths','patio','pause','peace','peach','peaks','pearl','pedal','peers','penis','penny','perks','pests','petty','phase','phone','photo','piano','picks','piece','piles','pills','pilot','pinch','pipes','pitch','pixel','pizza','place','plain','plane','plans','plant','plate','plays','plaza','plots','plugs','poems','point','poker','polar','poles','polls','pools','porch','pores','ports','posed','poses','posts','pouch','pound','power','press','price','pride','prime','print','prior','prize','probe','promo','prone','proof','props','proud','prove','proxy','psalm','pulls','pulse','pumps','punch','pupil','puppy','purse',
    ...
    'table','taken','takes','tales','talks','tanks','tapes','tasks','taste','tasty','taxes','teach','teams','tears','teens','teeth','tells','tempo','tends','tenth','tents','terms','tests','texts','thank','theft','their','theme','there','these','thick','thief','thigh','thing','think','third','those','three','threw','throw','thumb','tiger','tight','tiles','timer','times','tired','tires','title','toast','today','token','tones','tools','tooth','topic','torch','total','touch','tough','tours','towel','tower','towns','toxic','trace','track','tract','trade','trail','train','trait','trans','traps','trash','treat','trees','trend','trial','tribe','trick','tried','tries','trips','trout','truck','truly','trump','trunk','trust','truth','tubes','tumor','tuned','tunes','turbo','turns','tutor','tweet','twice','twins','twist','types','tyres',
    
    'ultra','uncle','under','union','unite','units','unity','until','upper','upset','urban','urged','urine','usage','users','using','usual',
    
    'vague','valid','value','valve','vapor','vault','vegan','veins','vents','venue','verse','video','views','villa','vinyl','viral','virus','visas','visit','vital','vivid','vocal','vodka','voice','volts','voted','voter','votes',
    
    'wages','wagon','waist','walks','walls','wants','warns','waste','watch','water','watts','waves','wears','weeds','weeks','weigh','weird','wells','welsh','whale','wheat','wheel','where','which','while','white','whole','whose','wider','widow','width','winds','wines','wings','wiped','wired','wires','witch','wives','woman','women','woods','words','works','world','worms','worry','worse','worst','worth','would','wound','wrath','wrist','write','wrong','wrote',
...
const keyboard = document.getElementById("keyboard").childNodes;
...
function initialize()
{
    ...
    for(let i = 0; i < 5; i++)
    {
        if(lettercount.has(currentWord.charAt(i)))
        {
            lettercount.set(currentWord.charAt(i), lettercount.get(currentWord.charAt(i))+1);
        }
        else
        lettercount.set(currentWord.charAt(i), 1);
    }
...
{   
    if(i!=10 && i!=20)
    {
        let name = keyboard[i*2+1].id;

        keyboard[i*2+1].onclick = function()
        {
            tempDraw(name);
            keyboard[i*2+1].blur();
        };

    keyboard[i*2+1].style.backgroundColor = "white";
    keyboard[i*2+1].display = "incline-block";
    keyboard[i*2+1].fontSize = "14px";
    keyboard[i*2+1].fontSize = "24px";
    ...
    if(!done)
        {
            if(cindex==5)
            {
                let s = "";
                let row = currentturn;
                let rowNodes = document.getElementById("grid").childNodes;
                let colNodes = rowNodes[row].childNodes;
                for(let i = 0; i <5; i++)
                {
                    s+=colNodes[i].innerHTML;
                }
                s = s.toLowerCase();
                guess(s);
            }
            ...
            {
                alert("Choose a five letter word!");
            }
        }
    ...
    if(cindex <=5 && cindex > 0)
        {
            ...
            let currentLoc = colNodes[cindex-1];
            currentLoc.innerHTML = " ";
            currentLoc.style.border = "3px solid hotpink";
            currentLoc.style.height = "75px";
            currentLoc.style.width = "60px";
            cindex--;
        }
...
function checkKeyPress(key)
{
    ...
    {
        case 97: 
        tempDraw(String.fromCharCode(temp));
        break;
        case 98: 
        tempDraw(String.fromCharCode(temp));
        break;
...
function tempDraw(letter)
{
    ...
    if(!done && cindex<5)
    {
        ...
        let colNodes = rowNodes[row].childNodes;
        //console.log(colNodes);
        let currentLoc = colNodes[cindex];
        currentLoc.style.fontWeight =  "bold";
        currentLoc.style.color = "purple";
        currentLoc.innerHTML = letter;
        currentLoc.style.border = "3px solid red";
        currentLoc.style.height = "75px";
        currentLoc.style.width = "60px";
        cindex++;
    }
...
function drawOriginal()
{
    ...
    {
        let x = document.getElementById("main").childNodes;
        let tempgrid = document.getElementById("grid");
        tempgrid.remove();
    }
    ...
    for(let i = 0; i < 6; i++)
    {
        ...
        for(let j = 0; j <5; j++)
        {
            ...
            col.style.border = "3px solid hotpink";
    ...
    table.style.border = "3px solid hotpink";
...
function reDraw()
{
    ...
    for(let i = 0; i < 6; i++)
    {
        ...
        {
            let temp = children[j];
            if(grid[i][j]==currentWord.charAt(j))
            { 
                let tempchar = grid[i][j];
                //tempchar = tempchar.toUpperCase();
                temp.style.backgroundColor = "green";
                document.getElementById(tempchar).style.backgroundColor = "green";
                greenButtons.add(tempchar);
        ...
        for(let k = 0; k <5; k++)
        {
            if(grid[i][k]!=currentWord.charAt(k))
            {
                good = false;
                break;
            }
    ...
    tableref.style.border = "3px solid hotpink";
    tableref.style.borderCollapse = "collapse";
    tableref.style.backgroundcolor = "green";
...
function reset()
{
    ...
    cindex = 0;
    initialize();
    document.getElementById("intro").innerHTML = "Wurdle Clone";
    document.getElementById("title").style.backgroundColor = "lime";
}



function guess(gWord)
{
    ...
    if(!done)
    {
        ...
        else
        {
            for(let i = 0; i <5; i++)
            {
                grid[currentturn][i] = gWord.substring(i, i+1);
            }
            currentturn++;
            reDraw();
            if(currentturn==6)
